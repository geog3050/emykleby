...
def fight(participant1, participant2, first2attack):
    ...
    while variable1 > 0 and variable2 > 0:
            if first2attack == 1:
                new_attack = (attack_multiplier(participant1[1],participant2[1])* participant1[3])
                variable2 = variable2 - new_attack
                first2attack = 2
            else:
                new_attack = (attack_multiplier(participant2[1],participant1[1])* participant2[3])
                variable1 = variable1 - new_attack
                first2attack = 1
            rounds +=1
            if variable1 > 0:
...
def tournament(participants):
    ...
    while i < len(participants):
       home_pokemon = participants[i]
       for away_pokemon in participants[(i+1):]:
           record.append(fight(home_pokemon,away_pokemon,1)[0])
           record.append(fight(home_fighter,away_pokemon,2)[0])
    i += 1
    for i in participants:
        wins.append(record.count(i[0]))
    return wins
